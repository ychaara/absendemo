...
class _AttendancePageState extends State<AttendancePage> {
  ...
  //ini izin pake kamera device nya
  ...
  //buat menyimpan hasil absen setelah pencocokan 
  Future<void> _recordAttendance(String name, String employeeCode) async {
    if (name.isEmpty || employeeCode.isEmpty) {
      _showError("Nama dan kode pegawai tidak boleh kosong");
      return;
    }

    DateTime now = DateTime.now();
    String date = "${now.year}-${now.month.toString().padLeft(2, '0')}-${now.day.toString().padLeft(2, '0')}";
    String entryTime = "${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')}:${now.second.toString().padLeft(2, '0')}";

    DateTime deadline = DateTime(now.year, now.month, now.day, 8, 0, 0);
    String status = now.isAfter(deadline) ? "2" : "1";
    String attendanceType = "in";
    String url = 'http://192.168.1.14/absendigital/absen/public/record_attendance';
    String nama = name;
    String kodePegawai = employeeCode;
    String tanggal = date;
    String jamMasuk = entryTime;
    String tipe_absen = attendanceType;
